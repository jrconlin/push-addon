...
if (false == true) {
/* TODO: add in hook for external plug-ins/apis 
 *
 * The idea being that this plugin would internally route
 * notifications to others that have registered interest. 
 * Obviously, end apps would have to do their own
 * decryption, since otherwise it's a huge security hole.
 *
 * This code is left in for cold storage, but should be 
 * removed for production. 
 */
const Unknown = require('api-utils/xpcom');

/* Sadly, although the sdk docs say you can do this, 
 * actual implementation says otherwise.
 *
 * Need to find out what the correct way to register to
 * xpcom's Unknown interface is.
 */
const NotifObserver = Unknown.extend({
  interfaces: ['nsIObserver'],
  topic: 'mpush_notification',
  initialize: function(cb) { this.cb = cb},
  register: function() {
  ...
  observe: function (subject, topic, data){
    // should we add additional info to the callback data?
    // (e.g. queueId, etc) or just pass back the data?
    this.cb({
      subject: subject,
      type: topic,
      data: data
...
NotificationApi.prototype = {
  ...
  checkRemotePermission: function(useCrypto) {
  ...
  getToken: function(cb, useCrypto) {
    let self = this;
    if (DB['token']) {
      return cb(DB['token']);
    }
    ...
    xhr({
      ...
      success: function(xhr) {
        ...
        if (!DB.sites.hasOwnProperty(hostname)) {
          DB.sites[hostname]={'token':queue,
            'name': hostname,
            'icon': 'resource://signal.png'
            };
        }
...
exports.main = function() {
  ...
  let intro = {
    ...
    time: new Date(),
    actionUrl: 'http://https://wiki.mozilla.org/Services/Notifications/Push',
    site: 'Welcome'
  };
