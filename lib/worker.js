...
exports.worker = function() {
  var worker = pageWorker.Page({
    ...
    contentScript: 'new ' + function() {
      ...
      self.port.on('socket', function(url) {
          console.log('worker:port.on.socket', url);
        socketURL = url;
      });
  ...
  worker.port.on('message', function(msg) {
      msg = JSON.parse(msg);
      // Add the site to the message.
      queueId = msg['queue'].replace(/\/$/,'').split('/').pop();
      msg['token'] = queueId;
      msg['site'] = main.DB['domain:' + queueId];
      console.debug('worker: message', msg);
      observer.emit('message', msg);
  });
